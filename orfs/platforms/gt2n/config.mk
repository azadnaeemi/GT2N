...
# Overridable (?=): the default M5 signal stack suits small designs (gcd);
# larger designs (AES) raise this per-flow to get enough routing layers.
export MAX_ROUTING_LAYER ?= M5

export FASTROUTE_TCL ?= $(PLATFORM_DIR)/fastroute.tcl

# OpenRCX parasitic-extraction model. Enables write_spef at finish so the
# final stage emits 6_final.spef (required by bazel-orfs's orfs_final).
